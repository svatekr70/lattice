...
function docFiltry(root, ctx) {
  build(root, [
    ...
    p('<b>Varování na neúplnou nabídku (v1.21.1)</b> — že se v režimu <code>serverSide</code> nabídka odvodila jen z načtené stránky, se dřív poznalo jen tím, že si někdo všiml chybějící hodnoty. Sloupec s filtrem <code>select</code>/<code>multiselect</code>/<code>multiselect-exclude</code>, který nemá ani <code>filterValues</code>, ani <code>filterUrl</code>, proto vypíše <code>console.warn</code> s názvem sloupce — jednou na sloupec a instanci, protože nabídka se přenačítá při každém otevření panelu. Platí i pro <code>progressiveLoad</code>: ten na dotažení zbytku dat nečeká. Client-side režim nevaruje, tam je odvození z celého datasetu úplné. U <code>multiselect-exclude</code> je hláška důraznější — filtr je inverzní, takže z neúplné nabídky uživatel vyloučí míň, než čeká, a přebývající řádky vypadají jako chyba filtru.'),
...
function docPresety(root) {
  build(root, [
    ...
    h3('Skupiny ve výběru (v1.20.0)'),
    p('Vedle názvu je při ukládání políčko <b>„Skupina…"</b> — volitelný štítek jako <i>Prodeje</i> nebo <i>Faktury</i>. Položky se stejným štítkem se v rozbalovacím výběru sdruží do <code>&lt;optgroup&gt;</code> pod jeho název (a v panelech pod nadpis skupiny); co skupinu nemá, zůstává nahoře. Platí pro uložené filtry i pohledy — dlouhá nabídka tak zůstane čitelná.'),
    p('Napovídání (<code>&lt;datalist&gt;</code>) nabízí už použité skupiny, zapsat jde ale cokoli. Skupiny se <b>nepřerovnávají</b> — jdou v pořadí prvního výskytu. Programově: <code>presets.saveLocal(name, parts, display, group)</code>, <code>presets.setGroup(preset, group)</code>, u filtrů <code>grid.saveAdvanced(name, tree, scope, display, group)</code>, <code>grid.saveFilterSnapshot(name, scope, display, group)</code>, <code>grid.setAdvancedGroup(id, group)</code> nebo <code>grid.renameSavedFilter(id, name, group)</code>. Do payloadu globálních callbacků chodí jako klíč <code>group</code>.'),
